...
""" Class for triggering a deletion in kubernetes """
...
class KubernetesDeleteActionState(Enum):
    """
    States for kubernetes delete action
    """
    IDLE = 1
...
class KubernetesDelete(py_trees.behaviour.Behaviour):
    """
    Class for triggering a deletion in kubernetes
    ...
    def __init__(self, target: str, namespace: str, element_type: str, grace_period: int, regex: bool):
        super().__init__('KubernetesDeleteAction')
        self.target = target
        self.namespace = namespace
        if element_type == "pod":
            self.element_type = ElementType.POD
        else:
