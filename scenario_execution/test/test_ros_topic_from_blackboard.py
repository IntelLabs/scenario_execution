...
class Test(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.pub = self.node.create_publisher(Bool, "/bla", 10)
        self.sub = self.node.create_subscription(Bool, "/final", self.callback, 10)
    ...
    def test_success(self):
        scenarios = self.parser.process_file(os.path.join(
            self.scenario_dir, 'scenarios', 'test', 'test_ros_topic_from_blackboard.osc'), False)
