...
def generate_test_description():
    ...
    return (
        LaunchDescription([
            ...
            #         scenario_execution_gazebo_dir, 'launch', 'second_robot_launch.py'))),
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(os.path.join(
                ...
                launch_arguments={'headless': 'True',
                                  ...
                                  'scenario': os.path.join(scenario_execution_gazebo_dir,
                                                           'scenarios', 'test_nav_to_pose.osc')}.items()),
        ]),
        {}
    )
...
class TestProcessOutput(unittest.TestCase):
    ...
    def test_output(self, proc_output, proc_info):
        # pylint: disable=missing-function-docstring
        proc_output.assertWaitFor("Executing scenario 'test_nav_to_pose'", timeout=120)
        ...
        proc_output.assertWaitFor("Scenario 'test_nav_to_pose' succeeded.", timeout=120)
