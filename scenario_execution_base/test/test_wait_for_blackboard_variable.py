...
class TestScenarioExectionSuccess(unittest.TestCase):
    ...
    def test_wait_for_blackboard_var(self):
        scenario_content = """
import osc.ros

scenario test_wait_for_blackboard_variable:
    do parallel:
        set_var: serial:
            wait elapsed(2s)
            balla: set_blackboard_variable() with:
                keep(it.variable_name == '/bla')
                keep(it.variable_value == 'hello')
        test: serial:
